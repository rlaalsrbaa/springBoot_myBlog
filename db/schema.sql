...
CREATE TABLE board (
    ...
    regDate DATETIME NOT NULL,
    updateDate DATETIME NOT NULL,
    `code` CHAR(50) NOT NULL UNIQUE COMMENT 'notice(공지사항),free(자유게시판1),tip&Tech(팁&테크),...',
...
select now(), now(), FLOOR(RAND() * 3) + 1, FLOOR(RAND() * 3) + 1, concat('제목_', rand()), CONCAT('내용_', RAND())
from article;
*/
